...
test('a valid commit message', async () => {
    const result = await lintMessage(':white_check_mark: a valid angular commit');
...
test('a valid message with a scope', async () => {
    const result = await lintMessage(
        ':white_check_mark: (scope) a valid angular commit with a scope'
    );
    expect(result.valid).toBe(true);
    expect(result.errors).toStrictEqual([]);
    expect(result.warnings).toStrictEqual([]);
...
test('an invalid scope', async () => {
    ...
    expect(result.errors[0].message).toBe(
        `type must be one of [${allGitmojiCodes.join(', ')}]`
    );
    expect(result.warnings).toStrictEqual([]);
});
...
test('a long header', async () => {
    const result = await lintMessage(
        `:white_check_mark: that its an error when there is ia realllllllllllllllllllllly long header`
    );

    expect(result.valid).toBe(false);
    expect(result.errors[0].message).toBe(
        'header must not be longer than 72 characters, current length is 92'
    );
    expect(result.warnings).toStrictEqual([]);
});
